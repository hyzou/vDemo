...
import distributeSuperviseTableHeader from "@/utils/formSettingsJson/informationsPage/specialAssessment/cityLeadTableHeader.json";

import selfEvaluationDialogFormItems from "@/utils/formSettingsJson/informationsPage/specialAssessment/selfEvaluationDialogFormItems.json";
import selfEvaluationDialogFormBtns from "@/utils/formSettingsJson/informationsPage/specialAssessment/selfEvaluationDialogFormBtns.json";
...
export default {
  ...
  methods: {
      ...
      if (flag.type === "distribute") {
        ...
        if (row.cityCoordinationDepartmentId) {
          ...
          if (
            ...
            !assessmentResult[row.targetId]["specialCityOpinion"] ||
            !assessmentResult[row.targetId]["specialCityOpinion"]["opinion"]
          ) {
            this.$message({
            ...
            if (assessmentResult[row.targetId]["specialCityOpinion"]) {
              let coordination =
                assessmentResult[row.targetId]["specialCityOpinion"];
    ...
    startOfficeSelf(row) {
      ...
      let values = { ...row, reduceScore: row.reduceScore || "N" };
    ...
    handleDialogFormSelfEvaluation(btns, formData) {
      ...
      if (formData.targetId) {
        this.$confirm("确认提交给考核办？提交后数据不能修改", "提交", {
          ...
          .then(confirm => {
            console.log(confirm);
            this.$store.getters["assessment/getSpecialAssessmentList"].forEach(
              item => {
                if (item.assessmentId === formData.assessmentId) {
                  ...
                  this.$store.dispatch(
                    "assessment/commitSpecialAssessmentInfo",
                    item
                  );
                }
              }
      ...
      tableDateSource.forEach(item => {
        ...
        if (item.cityLeadDepartmentId && item.cityLeadDepartmentId == userId) {
          ...
          if (
            ...
            assessmentResult[item.targetId].specialCitySelf.selfScore
          ) {
