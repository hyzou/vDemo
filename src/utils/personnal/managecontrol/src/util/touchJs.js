...
touch.onFunc = json => {
    ...
    obj.addEventListener("touchmove", e => {
        ...
        if (e.touches.length >= 2 && touch.isTouch) {
            ...
            if (getDistance(now[0], now[1]) < getDistance(touch.start[0], touch.start[1])) {
                if (typeof (scaleFunc) === "function") {
                    scaleFunc(0);
                }
            } else if (getDistance(now[0], now[1]) > getDistance(touch.start[0], touch.start[1])) {
                if (typeof (scaleFunc) === "function") {
                    scaleFunc(1);
                }
            };
    ...
    obj.addEventListener("touchend", e => {
        ...
        if (touch.isTouch) { touch.isTouch = false; }
    });
}
/* 更改图标移动旋转模式 */
