const methods = {
      ...
      case "reIN":
        ...
        this.$confirm("确定执行单据退货入库吗?", "提示", {
          ...
          this.$getData(this.$api.inOut_incomeCertify, {
            ...
            this.$message.success("您已成功执行单据退货入库!");
            this.initPageData();
          });
        });
        break;
   ...
   * 获取出库单列表
   */
  getOutList(id, inoutId) {
    return new Promise(reslove => {
      let filterRules = [
        { field: "type", op: "=", value: "OUT" },
        {
          field: "storepointId",
          op: "=",
          value: id
    ...
    return new Promise(reslove => {
      ...
      this.$getData(this.$api.detailList_incomeCertify, {
        ...
        toBechosedTalbeData.map(tableItem => {
          ...
          tableItem.canNum = tableItem.num - tableItem.stockNum;
          tableItem.num = tableItem.canNum;
          tableItem.finishNum = tableItem.canNum;
          tableItem.maxNumber = tableItem.canNum;
        });
