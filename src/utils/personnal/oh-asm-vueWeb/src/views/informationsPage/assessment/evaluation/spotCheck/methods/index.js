const methods = {
  ...
  formatData(data, prop) {
    data.map(item => {
      ...
      if (item.assessManageAssign) {
        item.assessManageOrg_dsp = item.assessManageAssign.assessedOrgId__dsp;
      }
      // 被考核牵头部门
  ...
  handleTableRowButton(row, btns) {
    ...
    // 默认满分
    defaultValues.spotCheck_score = defaultValues.spotCheck_score
      ? defaultValues.spotCheck_score
      : defaultValues.kpiScore;
    // 是否减分
    defaultValues["spotCheck_deducted"] = row["spotCheck_deducted"]
      ? row["spotCheck_deducted"]
      : "N";
    // // 是否引用日常监督文件
