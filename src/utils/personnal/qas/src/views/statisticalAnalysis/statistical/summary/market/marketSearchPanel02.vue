      ...
      <el-row>
        <el-col :span="21">
          ...
          <el-col :span="8">
            ...
            >
              <el-select
                ...
                @change="listRules"
              >
                <template v-for="item in $store.getters.admins">
                  <el-option
                    :key="item.value"
                    :label="item.text"
                    :value="item.value"
          ...
          <el-col :span="8">
            ...
            >
              <el-select v-model="search.marketType" placeholder="请选择">
                <template v-for="item in marketTypes">
                  <el-option
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
...
export default {
  ...
  methods: {
    ...
    findMarketTypes() {
      const $this = this;
      this.$get({
        url: "/_data/statistical/ruleSetting/findMarketCheckTypes",
        fnc: data => {
          if (!data.success || !data.data || data.data.length == 0) {
            $this.marketTypes = [];
            return false;
          }
          $this.marketTypes = data.data;
        }
      });
    },
    searchData() {
      ...
      this.$refs["summarySearchForm"].validate(valid => {
        ...
        $this.$Api.getReportServer().then(data => {
          ...
          if (menuId == this.$constants.QAS_PLAN_MARKET_STATISTICAL_CATALOG_5) {
            menuId = menuId + "_" + $this.search.marketType;
          }
          ...
          if ($this.search.marketType) {
            url += "&marketType=" + $this.search.marketType;
          }
          if ($this.search.adminLevel) {
            url += "&adminLevel=" + $this.search.adminLevel;
          }
