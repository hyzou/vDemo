  ...
  created() {
    ...
    if (this.formGroupSettings.validatePWRule) {
      ...
      this.formGroupSettings.formGroupList.forEach(formItem => {
        // 新增需要必填验证
        if (this.formGroupSettings.ruleType === "add") {
          // 密码验证
        ...
        // 编辑只做重复验证
        if (this.formGroupSettings.ruleType === "edit") {
          // 密码验证
          if (formItem.check === "pw") {
            validatePW2["function"] = (rule, value, callback) => {
              if (
      ...
      if (this.formGroupSettings.ruleType === "add") {
        ...
        this.formGroupSettings.rules[validatePW2["name"]] = [
          {
            required: true,
            validator: validatePW2["function"],
            trigger: "blur"
          }
        ];
      }
      ...
      if (this.formGroupSettings.ruleType === "edit") {
        this.formGroupSettings.rules[validatePW["name"]] = [
          { validator: validatePW["function"], trigger: "blur" }
        ];
        this.formGroupSettings.rules[validatePW2["name"]] = [
          { validator: validatePW2["function"], trigger: "blur" }
        ];
      }
    }
  },
