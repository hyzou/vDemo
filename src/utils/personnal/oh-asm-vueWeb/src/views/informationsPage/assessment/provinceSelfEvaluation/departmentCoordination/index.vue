...
export default {
  ...
  methods: {
    ...
    handleTableRowButton(row, flag) {
      if (flag.type === "distribute") {
        ...
        this.dialogFormSettingsOpinion.dialogFormTitle = "配合意见";
      ...
      tableDateSource.forEach(item => {
        ...
        if (
          ...
          if (
            ...
            assessmentResult[item.targetId]["cityOpinion"].opinion
          ) {
  ...
  mounted() {
    this.setTableList();
    let list = this.$store.getters["assessment/getTargetList"];
