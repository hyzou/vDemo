  ...
  <div>
    ...
    <div>
      ...
      >
        ...
        <el-table-column prop="baseValue" label="基准值" width="180">
          ...
          <template slot-scope="scope">
            <!-- 需要判断是什么类型的 如果是下拉的则直接下拉数据 -->
            <el-select
              v-if="scope.row.dataType == 'O'"
              v-model="scope.row.baseValueStr"
              @change="setBaseValue(scope.row)"
            >
              <el-option
                v-for="item in scope.row.dicList"
                :key="item.value"
                :label="item.text"
                :value="item.value"
              >
...
export default {
  ...
  methods: {
      ...
      this.$get({
        ...
        fnc: data => {
          ...
          dataMsg.basCheckitemList.forEach((item, index) => {
            ...
            if (item.baseValue) {
              item.baseValueStr = item.baseValue + "";
            }
    ...
    setBaseValue(scopeRow) {
      scopeRow.baseValue = scopeRow.baseValueStr;
    }
  },
  props: {
    ruleId: {
