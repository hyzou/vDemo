...
// 配置数据引入
import searchFormBtnList from "@/utils/formSettingsJson/measureUnitMng/searchFormBtns.json";
import searchFormItemslist from "@/utils/formSettingsJson/measureUnitMng/searchFormItems.json";
import tableOperationsBtnList from "@/utils/formSettingsJson/measureUnitMng/operationBtns.json";
import tableHeaderOpts from "@/utils/formSettingsJson/measureUnitMng/tableHeaderOpt.json";
import dialogEditFormItemslist from "@/utils/formSettingsJson/measureUnitMng/dialogFormItems.json";
import dialogFormAddBtnlist from "@/utils/formSettingsJson/measureUnitMng/dialogAddFormBtns.json";
import dialogFormEditBtnlist from "@/utils/formSettingsJson/measureUnitMng/dialogEditFormBtns.json";
import dialogViewFormItems from "@/utils/formSettingsJson/measureUnitMng/dialogViewFormItems.json";
// 配置表单数据校验规则
import dialogFormRules from "@/utils/formRulesJson/measureUnitMng/dialogRule";
  ...
  methods: {
    ...
    handleTableItem(id) {
      ...
      that.tablelistSettings.tableDatas.map(tabledata => {
        if (tabledata.sysCode == id) {
          that.dialogFormSettings.dialogFormTitle = "查看计量单位";
    ...
    handleTableRowButton(id, type) {
      ...
      if (type == "edit") {
        ...
        that.dialogFormSettings.dialogFormTitle = "编辑计量单位";
        ...
        this.$confirm("您正在执行删除操作，是否继续?", "提示", {
          ...
          that.$http(that.$api.removeUnit(), postdata).then(xhr => {
            if (xhr.success) {
              that.$message({
                message: "操作成功！",
                type: "success"
