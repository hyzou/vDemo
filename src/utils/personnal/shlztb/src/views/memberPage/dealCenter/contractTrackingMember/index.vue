...
export default {
  ...
  methods: {
    ...
    getDataList(query) {
      ...
      that.$getData("getMemberContractAttractList", postdata).then(data => {
        if (
          that.page > Math.ceil(data.total / that.$global.numberPerpage) &&
          that.page > 1
        ) {
          postdata.page = postdata.page - 1;
          that.$getData("getMemberContractAttractList", postdata).then(xhr => {
            that.dealData(xhr);
          });
        } else {
          that.dealData(data);
    ...
    dealData(data) {
      if (data.rows && data.rows.length > 0) {
        data.rows.map(item => {
          ...
          } else if (item.type == "02" || item.type == "11") {
            if (item.contractList && item.contractList.length > 0) {
              item.contractList.map(contract => {
                contract.isfinish = "已确认";
                contract.sendnumUnit =
