...
const methods = {
  ...
  handleOperateButton(data) {
    if (data.flag === "add") {
      ...
      this.dialogFormSettings.dialogFormItems.formGroupValues = {
        ...
        sourceType: this.$store.getters["user/baseInfos"].sourceTypeList[0]
          .value,
    ...
    if (data.flag === "addDetail") {
      if (
        ...
        this.tableToBeChosedReferSettings.tableDatas = [];
        if (
          this.dialogFormSettings.dialogFormItems.formGroupValues.sourceType ==
          "PP"
        ) {
    ...
    switch (btns.type) {
      case "edit":
        ...
        this.getSingleInfo(row.emmsDocInoutId).then(xhr => {
          ...
          this.contentItems.map(item => {
            if (item.name == "sourceId") {
              item.hideItem = xhr.data.sourceType == "NON" ? true : false;
              item.data = [];
              if (xhr.data.sourceType == "PP") {
                this.getPurchaseList(xhr.data.storepointId).then(listxhr => {
                  item.data = listxhr;
                });
              }
              if (xhr.data.sourceType == "RECOVER") {
                this.getRecoverList(
                  xhr.data.storepointId,
                  xhr.data.emmsDocInoutId
                ).then(listxhr => {
                  console.log(listxhr, "listxhr");
                  item.data = listxhr;
                });
              }
            }
  ...
  getMaterialDetail(id) {
    return new Promise(reslove => {
      this.$getData(this.$api.singleMaterialData, { id: id }).then(res => {
        ...
        res.data.storehouseId = "";
        res.data.wmsBasStackId = "";
  ...
  getRecoverList(storepointId, inoutId) {
    ...
    return new Promise(reslove => {
      this.$getData(this.$api.getRecoverListNopage, {
        filterRules: JSON.stringify(filterRules)
      }).then(res => {
   ...
   * 获取单条回收单详情
   */
  getSingleRecoverDetailInfo(id) {
    return new Promise(reslove => {
      this.$getData(this.$api.singleRecoverCertifyData, {
        emmsImplRecoverId: id
          ? id
   ...
   * 获取回收单物资列表
   */
  getSingleRecoverList(id) {
    let filterRules = [
      {
        field: "emmsImplRecoverId",
        op: "=",
        value: id
          ? id
          : this.dialogFormSettings.dialogFormItems.formGroupValues.sourceId
      ...
      {
        field: "emmsImplRecover.storepointId",
        op: "=",
    ...
    return new Promise(reslove => {
      this.$getData(this.$api.getRecoverDetaillist, {
        filterRules: JSON.stringify(filterRules),
        isReturnOperableNum: "Y",
        page: this.beChosedPaginationReferSettings.currentPage,
        rows: this.$global.paginationOption.numberPerpage
      }).then(res => {
        res.rows.map(row => {
          row.id = row.emmsImplRecoverDtlId;
          row.sourceType = row.emmsImplRecover.sourceType;
          row.sourceId = row.emmsImplRecover.inoutDocNo;
          row.sourceDtlId = row.emmsImplRecoverDtlId;
          row.storepointName = row.emmsImplRecover.storepointName;
          row.created = this.$globalFnc.dateStampFormat(
            row.emmsImplRecover.created
          );
          row.materialTypeName = row.emmsBasMaterial.basMaterialType.name;
          row.code = row.emmsBasMaterial.code;
          row.name = row.emmsBasMaterial.name;
          row.unitName = row.emmsBasMaterial.basUnit.name;
          // row.emmsBasMaterialSpecId =
          //   row.emmsBasMaterialSpec.emmsBasMaterialSpecId;
          row.specs = row.emmsBasMaterialSpec.specs;
          row.mType = row.emmsBasMaterialSpec.mType;
          row.storableNum = row.recoverableNum;
          row.num = row.recoverableNum;
          row.finishNum = 0;
          row.maxNumber = row.recoverableNum;
        });
   ...
   * 获取购置单列表
   */
  getPurchaseList(storepointId) {
    return new Promise(reslove => {
      this.$getData(this.$api.getPurchaseListNoPage, {
        isHasChild: "Y",
   ...
   * 获取单条购置单详情
   */
  getSinglePurchaseDetailInfo(id) {
    return new Promise(reslove => {
   ...
   * 获取购置单物资列表
   */
  getSinglePurchaseList(id) {
    let docNo = "";
    this.ordersList.map(item => {
      if (
        item.emmsPlanPurchaseId ==
        this.dialogFormSettings.dialogFormItems.formGroupValues.sourceId
      ) {
        docNo = item.docNo;
      }
    ...
    return new Promise(reslove => {
      this.$getData(this.$api.purchaseDtlList, {
        docNo: id ? id : docNo,
        page: this.beChosedPaginationReferSettings.currentPage,
        rows: this.$global.paginationOption.numberPerpage
      }).then(res => {
        res.rows.map(row => {
          row.id = row.emmsPlanPurchaseDtlId;
          row.sourceType = row.emmsPlanPurchase.sourceType__dsp;
          row.sourceId = row.emmsPlanPurchase.sourceDocNo;
          row.sourceDtlId = row.emmsPlanPurchaseDtlId;
          row.storepointName = row.emmsPlanPurchase.storepointId__dsp;
          row.created = this.$globalFnc.dateStampFormat(
            row.emmsPlanPurchase.created
          );
          row.materialTypeName = row.basMaterial.basMaterialType.name;
          row.code = row.basMaterial.code;
          row.name = row.basMaterial.name;
          row.unitName = row.basMaterial.basUnit.name;
          row.emmsBasMaterialSpecId = row.basMaterialSpec.emmsBasMaterialSpecId;
          row.specs = row.basMaterialSpec.specs;
          row.mType = row.basMaterialSpec.mType;
          row.num = row.storableNum;
          row.finishNum = 0;
          row.maxNumber = row.storableNum;
        });
        this.tableToBeChosedReferSettings.tableDatas = res.rows;
        this.beChosedPaginationReferSettings.totalNumbers = res.total;
        reslove(res);
  ...
  getStoreHouseList(id) {
    return new Promise(reslove => {
      this.$getData(this.$api.dictList_common, {
        _refKey: "storehouse",
        status: "1",
        spId: id ? id : this.$store.getters["user/userInfos"].storePointId
      }).then(res => {
  ...
  //计算table高度
  _getTableHeight() {
    let tableHeightDetil =
      this.$refs.mainPage.offsetHeight -
      this.$refs.headerInfos.offsetHeight -
      52;
    if (tableHeightDetil <= 300) {
      this.tableSettings.tableSettingOptions.height = 300;
    } else {
      this.tableSettings.tableSettingOptions.height = tableHeightDetil;
    }
    // console.log(
    //   this.tableSettings.tableSettingOptions.height,
    //   tableHeightDetil,
    //   this.$refs,
    //   this.$refs.mainPage.offsetHeight,
    //   this.$refs.headerInfos.offsetHeight
