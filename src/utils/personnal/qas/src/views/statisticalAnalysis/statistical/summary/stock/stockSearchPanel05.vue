      ...
      <el-row>
        <el-col :span="21">
          ...
          <el-col :span="8">
            ...
            >
              <el-select
                v-model="search.overPassResult"
                placeholder="请选择"
                multiple
              >
                <el-option
                  v-for="item in overPassResultSource"
                  :key="item.value"
                  :label="item.text"
                  :value="item.value"
                />
          ...
          <el-col :span="8" v-if="showPointName">
          ...
          <el-col :span="8" v-if="showDistrict">
  ...
  methods: {
    ...
    findPassResultSource() {
      ...
      this.$Api.getDic("wms_bas_qualcheckresult").then(data => {
        $this.overPassResultSource = data;
      });
    },
    changeDORS(newValue) {
    ...
    searchData() {
      ...
      this.$refs["summarySearchForm"].validate(valid => {
        ...
        $this.$Api.getReportServer().then(data => {
          ...
          if (
            $this.search.overPassResult &&
            $this.search.overPassResult.length > 0
          ) {
            url += "&passResults=" + $this.search.overPassResult.join(",");
          }
